...
Para estimar el coeficiente de Gini no tenemos que crear vectores propiamente, 
...
gini_por_sector <- datos %>%
  group_by(cve_sec) %>%
  summarise(gini = Gini(po, na.rm = TRUE))

print(gini_por_sector)
```

```{r}
# Calcular el índice de Gini por entidad
gini_por_entidad <- datos %>%
  group_by(cve_ent) %>%
  summarise(gini = Gini(po, na.rm = TRUE))

print(gini_por_entidad)
```
