...
print(gini_sector)
```
Para el segundo caso, la estimación del coeficiente de Gini para el conjunto de 32 entidades federativas requiere filtrar los datos y escoger el sector de interés. Se crea un objeto llamado`datos_31_32` que contiene los datos del sector manufacturero y posteriormente se calcula el coeficiente de gini agrupando por `nom_ent`.

```{r}
# Filtrar solo los registros donde sve_sec es "31-33"
datos_31_33 <- datos %>% filter(cve_sec == "31-33")

# Calcular el índice de Gini por entidad
gini_entidad <- datos_31_33 %>%
  group_by(nom_ent) %>%
  summarise(gini = Gini(po,  na.rm = TRUE))
